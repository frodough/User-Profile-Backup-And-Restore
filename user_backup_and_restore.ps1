...
## Function from "https://devblogs.microsoft.com/scripting/build-a-better-copy-item-cmdlet-2/"
function Copy-WithProgress {
    ...
    foreach ($File in $Filelist)
    {
        ...
        Copy-Item $File.FullName -Destination $DestinationFile -ErrorAction SilentlyContinue
        $Position++
    }
}
    ...
    if ("y","yes" -contains $Choice) {
        ...
        foreach ($Folder in $Folders) {
            ...
            if ($valid) {
                ...
                try {
                    Copy-WithProgress -Source $("$env:homedrive\Users\$UserBackup\$Folder") -Destination "$($Destination)\$($UserBackup)\$Folder"
    ...
    if ("y","yes" -contains $Choice) {
        ...
        foreach ($Folder in $Folders) {
            ...
            if ($valid) {
				...
				## Try to copy files, if error occurs stop processing and display error
                try {
                    Copy-WithProgress -Source "$($Destination)\$($UserBackup)\$Folder" -Destination $("$env:homedrive\Users\$UserBackup\$Folder") 
